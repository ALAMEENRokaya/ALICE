...
def train(model, train_loader, optimizer, loss_function, device, lmbda, max_iters=20_000):
    model.train()
...
def validate(model, val_loader, loss_function, device, lmbda):
    model.eval()
    running = {"loss": 0, "mse": 0, "bpp": 0}
    n = 0
    set_bitrate(model, BITRATE_IDX)
...
def train_model(model, device, train_loader, val_loader, args,
    ...
    for epoch in range(epochs):
        train_stats = train(model, train_loader, optimizer, loss_fn, device, args.lmbda)
        val_stats   = validate(model, val_loader, loss_fn, device, args.lmbda)
        ...
        kodak_metrics["lmbda"]       = args.lmbda
        print(
            f"Kodak idx={BITRATE_IDX} (λ={args.lmbda}) | "
...
def parse_args(argv):
    parser = argparse.ArgumentParser(description="Adapter fine-tuning at a given lambda.")
    parser.add_argument("-d", "--dataset", type=str, required=True,
                        help="Training dataset")
    parser.add_argument("--lmbda", type=float, required=True,
                        choices=[0.0018, 0.0035, 0.0067, 0.013, 0.025, 0.0483],
                        help="Lambda for the rate-distortion loss")
    ...
    parser.add_argument("--save-dir", type=str, default=None,
                        help="Where to save checkpoints (default: checkpoints_adapter_<lmbda>)")
...
def main(argv):
    ...
    if args.save_dir is None:
        lmbda_str = str(args.lmbda).replace(".", "")
        args.save_dir = f"checkpoints_adapter_{lmbda_str}"
    print(args)

    wandb.init(
        project="image-compression-training",
        name=f"adapter_lmbda{args.lmbda}_bs{args.batch_size}",
        config=vars(args),
    )
    ...
    print("CUDA device name:", torch.cuda.get_device_name(args.cuda_device))
